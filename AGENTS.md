...
*   `src/`: Core implementation modules
    ...
    *   `sessions.ts`: Persisted mapping of `pi` session IDs to agy conversation/session IDs. Engine-scoped KEYS (`sid:<x>` legacy, `sid:<x>@acp`) so engine switches never cross conversations.
...
*   **Sessions**: engine-scoped keys (`sid:<x>` legacy, `sid:<x>@acp`) — engine switches never cross conversations and rollback preserves both bindings.
